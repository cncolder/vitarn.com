...
var locale = exports.locale = {
  ...
  zh: {
    ...
    'Information technology will be anytime, anywhere in the future. Same as air.': '信息技术终将如同空气一样随时随地围绕着您.',
    'Find Out More': '探索更多',
    ...
    regex: [[/User already exists with name (.+)/, '用户名 $1 已存在']]
  }
};

exports.default = function (lang) {
  return function (literal, value) {
    var loc = locale[lang] || locale.en;

    // placeholder empty object
    if (!literal) return '';

    // one by one
    var label = loc && loc[literal];
    if (label && !value) return label;

    // regexp replace
    var regex = loc.regex && loc.regex.find(function (arr) {
      return arr[0].test(literal);
    });
    if (regex) return literal.replace(regex[0], regex[1]);

    // missing
    if ('en' != lang) log('missing locale %o %o', literal, value);
    return literal;
  };
};